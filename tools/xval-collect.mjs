...
// and confirms monoDip≈0 everywhere (the cold-open invariant — any nonzero is a regression to flag).
...
for (const f of fs.readdirSync(dir).filter(f => f.endsWith('.txt')).sort()) {
  ...
  rows.push({ file: f, ...kv, mono: parseFloat(kv.monoDip), deficit: parseFloat(kv.diagWorst), T: +kv.T });
}
...
console.log('| kit | class | T | fight-band | diag deficit | monoDip |');
console.log('|-----|-------|---|-----------|--------------|---------|');
for (const r of defs) console.log(`| ${r.kit} | ${r.class} | ${r.T} | ${band(r.T)} | **${r.diagWorst}** | ${r.monoDip} |`);
console.log(`\n### Clean tables\n`);
