...
needs judgment.** _(A full-set adversarial re-run is in flight; numbers below are from the collector +
the 18-table adversarial pass, to be reconciled when it returns.)_

**Headline (36/36):**
- **monoDip = 0.00% on EVERY table** — invariant A holds everywhere (the 18-table adversarial pass found
  **zero** row-dips; the full-set re-run is confirming the other 18). NOTE: *necessary, not sufficient* —
...
- **Every fight-class (non-boss) deficit is sub-1%** — worst **0.77%** (isc+scb medium). 1 table CLEAN,
  the other 29 sub-1% DEFICIT.
...
**Bottom line (data-complete, verdict pending the full-set adversary):** invariant A (monoDip=0) holds
on all 36. On invariant B, the non-KT deficits are all sub-1%; the one that is clearly **real and
length-persistent is `mqg+skull`'s low-haste slack** (the acceptance-test blocker), plus a mid/high-haste
scatter in the scb kits (§4.2) to triage; the rest reverse/vanish with length (quantization). KT is
excluded (AoE limitation). So the model **does not yet fully pass** `docs/ACCEPTANCE.md` — the fix pass
owns it. The raw matrices under `tools/xval-results/` are the record; the collector output is the
