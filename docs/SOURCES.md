...
| Ashtongue Talisman of Insight | spell crits have a **0.50** chance to grant **+145** spell haste rating for **5.0 s** (5000 ms), **no ICD** | Wowhead item 32488 (buff spell 40482, "Insight of the Ashtongue") + wowsims mage item proc (`ProcChance 0.5`, no internal cooldown — MODEL-DEFECTS §9b "Verified CLEAN") + `sim/core/unit.go:501` (145 additive into the one rating pool, like Drums/Skull/MQG) | **verified** — the proc rate is crit-driven and the granted haste is a rating source, so it feeds back into its own attempt rate; modeled exactly by the renewal law (ESTABLISHED-FACTS §12, `GAME.ATI`). Proc chance per cast uses the EFFECTIVE crit incl. the Clearcasting→Arcane Potency +3 pp lift (§9b: 0 rank flips without ATI, 3849 with). |
...
| Tirisfal Regalia (T5) 2pc | **+20% Arcane Blast damage** (and +20% AB mana cost) — AB only, not AE | wowsims `mage/items.go` (set 649, `SpellMod_DamageDone_Flat` .20, `ClassMask MageSpellArcaneBlast`) + per-hit log | **verified** — the reference export wears it: per-hit AB/AE ratio 2.99 (vs 2.52 without); solving per-hit data gives SP≈1414 ≈ the 1387 input. **Now an input toggle** (`ck-t5`, user-directed): ×1.2 on AB damage + normalizations (single-target invariant) and +20% AB mana in the chip; on, the AoE thresholds of RULES §9 shift ×1.2 (`M_eff = M/1.2`). AP pooling ADDITIVE per the row below. |
| Arcane Power × Tirisfal-2pc stacking | wowsims: **additive** (AB under AP+T5 = ×1.5, so AP = +25% relative on a T5'd AB, full +30% on AE) | wowsims `arcane_power.go` + `core/spell_mod.go` (both `SpellMod_DamageDone_Flat` → one additive pool) + CRN-paired measurement (TOOLING) | **verified in wowsims; RESOLVED as ADDITIVE for the model** (user ruling: trust wowsims when public 2.4.3 sources are silent; both effects are percent-damage aura modifiers that sum in the client's pool). The `ck-t5` toggle pools additively (`dmgMult + t5add`). |
