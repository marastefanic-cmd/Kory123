...
which closed without starting them either — they went to **PHASE13 §5.3 (`18-…` here), now `docs/ROADMAP.md` §0d's**. ⛔ And they need
...
calls verbatim, was inherited by PHASE13 §5/§6 (`18-…` here), now `docs/ROADMAP.md` §0d.
...
first time) and the acceptance re-gather — both went to PHASE13 (`18-…` here) — the programme now lives at `docs/ROADMAP.md` §0d; the re-gather was voided with the sim. ⚠ The **AoE edge** it left open was
...
**`docs/ROADMAP.md` §0d carries what it left open** (the constructive-enumeration programme, the
platform track, the seven live user calls, the §8 traps). What closed it: §1 landed 07-27 (the
...
_(No plan doc is in flight: **`docs/ROADMAP.md`** is the live plan. If a future phase opens its own
doc, archive it here when it closes.)_
