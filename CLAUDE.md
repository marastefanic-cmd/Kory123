...
- `docs/ESTABLISHED-FACTS.md` — ★ **how every cooldown actually behaves, measured, one at a time.**
  Read it before arguing placement from first principles: this project has twice explained a correct
  measurement with an incorrect mechanism. Per spell, with its own datalog at each baseline (h=0/400/800
  so far). **Facts only** — three rules, the third being the one that bites: as passive haste approaches
  the ~789 GCD cap a haste cooldown converts less and less at steady state and eventually **nothing**,
  while still converting at the pull where casts are longer than the floor. Damage cooldowns have no
  cap. Add baselines, never edit one in place. Generated by `tools/buff-atlas.mjs`, which refuses to
  print from an older engine.
