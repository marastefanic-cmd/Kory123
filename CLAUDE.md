...
- `docs/ESTABLISHED-FACTS.md` — ★ **the measured behaviour of every cooldown, one at a time, SIM next
  to MODEL.** Read it before arguing about placement from first principles: this project has twice
  explained a correct measurement with an incorrect mechanism, and a table settles what prose cannot.
  Baseline A (60 s · h=0 · 1000 SP · 25 % crit) is in; more baselines get **added**, never edited in
  place, so the vectors between them stay readable. Generated by `tools/buff-atlas.mjs`, which refuses
  to print from an older engine. It already carries two named anomalies: a **model** defect on
  Bloodlust (2.92 %, deterministic) and **sim noise** on Serpent-Coil (shrinks with iterations, so not
  a fact).
